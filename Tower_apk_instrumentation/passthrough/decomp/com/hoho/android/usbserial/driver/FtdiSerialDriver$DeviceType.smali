.class final enum Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;
.super Ljava/lang/Enum;
.source "FtdiSerialDriver.java"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lcom/hoho/android/usbserial/driver/FtdiSerialDriver;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x401a
    name = "DeviceType"
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "Ljava/lang/Enum",
        "<",
        "Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;",
        ">;"
    }
.end annotation


# static fields
.field private static final synthetic $VALUES:[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

.field public static final enum TYPE_2232C:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

.field public static final enum TYPE_2232H:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

.field public static final enum TYPE_4232H:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

.field public static final enum TYPE_AM:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

.field public static final enum TYPE_BM:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

.field public static final enum TYPE_R:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;


# direct methods
.method static constructor <clinit>()V
    .registers 4

    .line 155
    new-instance v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 155
    .local v0, "$r0":Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
    const-string v1, "TYPE_BM"

    .line 155
    const/4 v2, 0x0

    .line 155
    invoke-direct {v0, v1, v2}, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_BM:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    new-instance v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 155
    const-string v1, "TYPE_AM"

    .line 155
    const/4 v2, 0x1

    .line 155
    invoke-direct {v0, v1, v2}, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_AM:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    new-instance v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 155
    const-string v1, "TYPE_2232C"

    .line 155
    const/4 v2, 0x2

    .line 155
    invoke-direct {v0, v1, v2}, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_2232C:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    new-instance v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 155
    const-string v1, "TYPE_R"

    .line 155
    const/4 v2, 0x3

    .line 155
    invoke-direct {v0, v1, v2}, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_R:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    new-instance v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 155
    const-string v1, "TYPE_2232H"

    .line 155
    const/4 v2, 0x4

    .line 155
    invoke-direct {v0, v1, v2}, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_2232H:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    new-instance v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 155
    const-string v1, "TYPE_4232H"

    .line 155
    const/4 v2, 0x5

    .line 155
    invoke-direct {v0, v1, v2}, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_4232H:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x6

    new-array v3, v2, [Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .local v3, "$r1":[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_BM:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x0

    aput-object v0, v3, v2

    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_AM:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x1

    aput-object v0, v3, v2

    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_2232C:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x2

    aput-object v0, v3, v2

    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_R:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x3

    aput-object v0, v3, v2

    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_2232H:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x4

    aput-object v0, v3, v2

    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->TYPE_4232H:Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    const/4 v2, 0x5

    aput-object v0, v3, v2

    sput-object v3, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->$VALUES:[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    return-void
    .end local v3    # "$r1":[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
    .end local v0    # "$r0":Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
.end method

.method private constructor <init>(Ljava/lang/String;I)V
    .registers 3
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()V"
        }
    .end annotation

    .line 154
    invoke-direct {p0, p1, p2}, Ljava/lang/Enum;-><init>(Ljava/lang/String;I)V

    return-void
.end method

.method public static valueOf(Ljava/lang/String;)Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;
    .registers 5
    .param p0, "name"    # Ljava/lang/String;

    .line 154
    const-class v1, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 154
    invoke-static {v1, p0}, Ljava/lang/Enum;->valueOf(Ljava/lang/Class;Ljava/lang/String;)Ljava/lang/Enum;

    move-result-object v0

    .local v0, "$r1":Ljava/lang/Enum;, ""
    move-object v3, v0

    check-cast v3, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    move-object v2, v3

    .local v2, "$r2":Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
    return-object v2
    .end local v0    # "$r1":Ljava/lang/Enum;, ""
    .end local v2    # "$r2":Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
.end method

.method public static values()[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;
    .registers 3

    .line 154
    sget-object v0, Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;->$VALUES:[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    .line 154
    .local v0, "$r1":[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
    invoke-virtual {v0}, Ljava/lang/Object;->clone()Ljava/lang/Object;

    move-result-object v1

    .local v1, "$r0":Ljava/lang/Object;, ""
    move-object v2, v1

    check-cast v2, [Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;

    move-object v0, v2

    return-object v0
    .end local v0    # "$r1":[Lcom/hoho/android/usbserial/driver/FtdiSerialDriver$DeviceType;, ""
    .end local v1    # "$r0":Ljava/lang/Object;, ""
.end method
