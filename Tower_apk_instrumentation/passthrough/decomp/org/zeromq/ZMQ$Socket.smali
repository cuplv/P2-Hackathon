.class public Lorg/zeromq/ZMQ$Socket;
.super Ljava/lang/Object;
.source "ZMQ.java"

# interfaces
.implements Ljava/io/Closeable;


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lorg/zeromq/ZMQ;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x9
    name = "Socket"
.end annotation


# static fields
.field private static final DYNFROM:I = 0xc000

.field private static final DYNTO:I = 0xffff


# instance fields
.field private final base:Lzmq/SocketBase;

.field private final ctx:Lzmq/Ctx;


# direct methods
.method protected constructor <init>(Lorg/zeromq/ZMQ$Context;I)V
    .registers 5
    .param p1, "context"    # Lorg/zeromq/ZMQ$Context;
    .param p2, "type"    # I

    .line 361
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 362
    invoke-static {p1}, Lorg/zeromq/ZMQ$Context;->access$000(Lorg/zeromq/ZMQ$Context;)Lzmq/Ctx;

    move-result-object v0

    .local v0, "$r2":Lzmq/Ctx;, ""
    iput-object v0, p0, Lorg/zeromq/ZMQ$Socket;->ctx:Lzmq/Ctx;

    .line 363
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->ctx:Lzmq/Ctx;

    .line 363
    invoke-virtual {v0, p2}, Lzmq/Ctx;->create_socket(I)Lzmq/SocketBase;

    move-result-object v1

    .local v1, "$r3":Lzmq/SocketBase;, ""
    iput-object v1, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 364
    return-void
    .end local v1    # "$r3":Lzmq/SocketBase;, ""
    .end local v0    # "$r2":Lzmq/Ctx;, ""
.end method

.method protected constructor <init>(Lzmq/SocketBase;)V
    .registers 3
    .param p1, "base_"    # Lzmq/SocketBase;

    .line 366
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/4 v0, 0x0

    iput-object v0, p0, Lorg/zeromq/ZMQ$Socket;->ctx:Lzmq/Ctx;

    .line 368
    iput-object p1, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 369
    return-void
.end method

.method static synthetic access$100(Lorg/zeromq/ZMQ$Socket;)Lzmq/SocketBase;
    .registers 2
    .param p0, "x0"    # Lorg/zeromq/ZMQ$Socket;

    .line 343
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .local v0, "r1":Lzmq/SocketBase;, ""
    return-object v0
    .end local v0    # "r1":Lzmq/SocketBase;, ""
.end method

.method private final bind(Ljava/lang/String;II)I
    .registers 12
    .param p1, "addr"    # Ljava/lang/String;
    .param p2, "min"    # I
    .param p3, "max"    # I

    .line 1095
    const-string v1, ":*"

    .line 1095
    invoke-virtual {p1, v1}, Ljava/lang/String;->endsWith(Ljava/lang/String;)Z

    move-result v0

    .local v0, "$z0":Z, ""
    if-eqz v0, :cond_34

    .line 1097
    const/16 v3, 0x3a

    .line 1097
    invoke-virtual {p1, v3}, Ljava/lang/String;->lastIndexOf(I)I

    move-result v2

    .local v2, "$i2":I, ""
    add-int/lit8 v2, v2, 0x1

    .line 1097
    const/4 v3, 0x0

    .line 1097
    invoke-virtual {p1, v3, v2}, Ljava/lang/String;->substring(II)Ljava/lang/String;

    move-result-object p1

    .local p1, "$r1":Ljava/lang/String;, ""
    :goto_15
    if-gt p2, p3, :cond_4d

    .line 1099
    new-instance v4, Ljava/lang/StringBuilder;

    .line 1099
    .local v4, "$r2":Ljava/lang/StringBuilder;, ""
    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    .line 1099
    invoke-virtual {v4, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v4

    .line 1099
    invoke-virtual {v4, p2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v4

    .line 1099
    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v5

    .line 1101
    .local v5, "$r3":Ljava/lang/String;, ""
    iget-object v6, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1101
    .local v6, "$r4":Lzmq/SocketBase;, ""
    invoke-virtual {v6, v5}, Lzmq/SocketBase;->bind(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_31

    .line 1117
    return p2

    .line 1103
    :cond_31
    add-int/lit8 p2, p2, 0x1

    .local p2, "$i0":I, ""
    goto :goto_15

    .line 1106
    :cond_34
    iget-object v6, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1106
    invoke-virtual {v6, p1}, Lzmq/SocketBase;->bind(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_4d

    .line 1109
    :try_start_3c
    const/16 v3, 0x3a

    .line 1109
    invoke-virtual {p1, v3}, Ljava/lang/String;->lastIndexOf(I)I

    move-result p2
    :try_end_42
    .catch Ljava/lang/NumberFormatException; {:try_start_3c .. :try_end_42} :catch_52

    add-int/lit8 p2, p2, 0x1

    .line 1109
    :try_start_44
    invoke-virtual {p1, p2}, Ljava/lang/String;->substring(I)Ljava/lang/String;

    move-result-object p1

    .line 1109
    invoke-static {p1}, Ljava/lang/Integer;->parseInt(Ljava/lang/String;)I

    move-result p2
    :try_end_4c
    .catch Ljava/lang/NumberFormatException; {:try_start_44 .. :try_end_4c} :catch_52

    return p2

    .line 1116
    :cond_4d
    invoke-direct {p0}, Lorg/zeromq/ZMQ$Socket;->mayRaise()V

    const/4 v3, -0x1

    return v3

    .line 1111
    :catch_52
    move-exception v7

    .local v7, "$r5":Ljava/lang/NumberFormatException;, ""
    const/4 v3, 0x0

    return v3
    .end local p2    # "$i0":I, ""
    .end local v6    # "$r4":Lzmq/SocketBase;, ""
    .end local p1    # "$r1":Ljava/lang/String;, ""
    .end local v4    # "$r2":Ljava/lang/StringBuilder;, ""
    .end local v7    # "$r5":Ljava/lang/NumberFormatException;, ""
    .end local v2    # "$i2":I, ""
    .end local v5    # "$r3":Ljava/lang/String;, ""
    .end local v0    # "$z0":Z, ""
.end method

.method private mayRaise()V
    .registers 5

    .line 1342
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1342
    .local v0, "$r1":Lzmq/SocketBase;, ""
    invoke-virtual {v0}, Lzmq/SocketBase;->errno()I

    move-result v1

    .local v1, "$i0":I, ""
    if-eqz v1, :cond_12

    const/16 v2, 0x23

    if-eq v1, v2, :cond_12

    .line 1344
    new-instance v3, Lorg/zeromq/ZMQException;

    .line 1344
    .local v3, "$r2":Lorg/zeromq/ZMQException;, ""
    invoke-direct {v3, v1}, Lorg/zeromq/ZMQException;-><init>(I)V

    throw v3

    :cond_12
    return-void
    .end local v3    # "$r2":Lorg/zeromq/ZMQException;, ""
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method private setsockopt(ILjava/lang/Object;)V
    .registers 5
    .param p1, "option"    # I
    .param p2, "value"    # Ljava/lang/Object;

    .line 411
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 411
    .local v0, "$r2":Lzmq/SocketBase;, ""
    :try_start_2
    invoke-virtual {v0, p1, p2}, Lzmq/SocketBase;->setsockopt(ILjava/lang/Object;)V
    :try_end_5
    .catch Lzmq/ZError$CtxTerminatedException; {:try_start_2 .. :try_end_5} :catch_6

    .line 413
    return-void

    .line 412
    :catch_6
    move-exception v1

    .local v1, "$r3":Lzmq/ZError$CtxTerminatedException;, ""
    return-void
    .end local v1    # "$r3":Lzmq/ZError$CtxTerminatedException;, ""
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method


# virtual methods
.method public base()Lzmq/SocketBase;
    .registers 2

    .line 377
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .local v0, "r1":Lzmq/SocketBase;, ""
    return-object v0
    .end local v0    # "r1":Lzmq/SocketBase;, ""
.end method

.method public final bind(Ljava/lang/String;)I
    .registers 5
    .param p1, "addr"    # Ljava/lang/String;

    .line 1080
    const v1, 0xc000

    .line 1080
    const v2, 0xffff

    .line 1080
    invoke-direct {p0, p1, v1, v2}, Lorg/zeromq/ZMQ$Socket;->bind(Ljava/lang/String;II)I

    move-result v0

    .local v0, "$i0":I, ""
    return v0
    .end local v0    # "$i0":I, ""
.end method

.method public bindToRandomPort(Ljava/lang/String;)I
    .registers 7
    .param p1, "addr"    # Ljava/lang/String;

    .line 1129
    new-instance v0, Ljava/lang/StringBuilder;

    .line 1129
    .local v0, "$r2":Ljava/lang/StringBuilder;, ""
    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    .line 1129
    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    .line 1129
    const-string v1, ":*"

    .line 1129
    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    .line 1129
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    .line 1129
    .local p1, "$r1":Ljava/lang/String;, ""
    const v3, 0xc000

    .line 1129
    const v4, 0xffff

    .line 1129
    invoke-direct {p0, p1, v3, v4}, Lorg/zeromq/ZMQ$Socket;->bind(Ljava/lang/String;II)I

    move-result v2

    .local v2, "$i0":I, ""
    return v2
    .end local v0    # "$r2":Ljava/lang/StringBuilder;, ""
    .end local p1    # "$r1":Ljava/lang/String;, ""
    .end local v2    # "$i0":I, ""
.end method

.method public bindToRandomPort(Ljava/lang/String;II)I
    .registers 6
    .param p1, "addr"    # Ljava/lang/String;
    .param p2, "min"    # I
    .param p3, "max"    # I

    .line 1145
    new-instance v0, Ljava/lang/StringBuilder;

    .line 1145
    .local v0, "$r2":Ljava/lang/StringBuilder;, ""
    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    .line 1145
    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    .line 1145
    const-string v1, ":*"

    .line 1145
    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    .line 1145
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    .line 1145
    .local p1, "$r1":Ljava/lang/String;, ""
    invoke-direct {p0, p1, p2, p3}, Lorg/zeromq/ZMQ$Socket;->bind(Ljava/lang/String;II)I

    move-result p2

    .local p2, "$i0":I, ""
    return p2
    .end local p1    # "$r1":Ljava/lang/String;, ""
    .end local p2    # "$i0":I, ""
    .end local v0    # "$r2":Ljava/lang/StringBuilder;, ""
.end method

.method public close()V
    .registers 2

    .line 386
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 386
    .local v0, "$r1":Lzmq/SocketBase;, ""
    invoke-virtual {v0}, Lzmq/SocketBase;->close()V

    .line 387
    return-void
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final connect(Ljava/lang/String;)V
    .registers 3
    .param p1, "addr"    # Ljava/lang/String;

    .line 1156
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1156
    .local v0, "$r2":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p1}, Lzmq/SocketBase;->connect(Ljava/lang/String;)Z

    .line 1157
    invoke-direct {p0}, Lorg/zeromq/ZMQ$Socket;->mayRaise()V

    .line 1158
    return-void
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public final disconnect(Ljava/lang/String;)Z
    .registers 4
    .param p1, "addr"    # Ljava/lang/String;

    .line 1169
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1169
    .local v0, "$r2":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p1}, Lzmq/SocketBase;->term_endpoint(Ljava/lang/String;)Z

    move-result v1

    .local v1, "$z0":Z, ""
    return v1
    .end local v1    # "$z0":Z, ""
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public final getAffinity()J
    .registers 8

    .line 588
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 588
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/4 v2, 0x4

    .line 588
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockoptx(I)Ljava/lang/Object;

    move-result-object v1

    .local v1, "$r2":Ljava/lang/Object;, ""
    move-object v4, v1

    check-cast v4, Ljava/lang/Long;

    move-object v3, v4

    .line 588
    .local v3, "$r3":Ljava/lang/Long;, ""
    invoke-virtual {v3}, Ljava/lang/Long;->longValue()J

    move-result-wide v5

    .local v5, "$l0":J, ""
    return-wide v5
    .end local v5    # "$l0":J, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$r3":Ljava/lang/Long;, ""
    .end local v1    # "$r2":Ljava/lang/Object;, ""
.end method

.method public final getBacklog()J
    .registers 6

    .line 452
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 452
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x13

    .line 452
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
.end method

.method public getDelayAttachOnConnect()Z
    .registers 4

    .line 1056
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1056
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x27

    .line 1056
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    const/4 v2, 0x1

    if-ne v1, v2, :cond_d

    const/4 v2, 0x1

    return v2

    :cond_d
    const/4 v2, 0x0

    return v2
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getEvents()I
    .registers 4

    .line 940
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 940
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0xf

    .line 940
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    return v1
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final getFD()Ljava/nio/channels/SelectableChannel;
    .registers 6

    .line 929
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 929
    .local v0, "$r2":Lzmq/SocketBase;, ""
    const/16 v2, 0xe

    .line 929
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockoptx(I)Ljava/lang/Object;

    move-result-object v1

    .local v1, "$r1":Ljava/lang/Object;, ""
    move-object v4, v1

    check-cast v4, Ljava/nio/channels/SelectableChannel;

    move-object v3, v4

    .local v3, "$r3":Ljava/nio/channels/SelectableChannel;, ""
    return-object v3
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
    .end local v3    # "$r3":Ljava/nio/channels/SelectableChannel;, ""
    .end local v1    # "$r1":Ljava/lang/Object;, ""
.end method

.method public final getHWM()J
    .registers 3
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    const-wide/16 v0, -0x1

    return-wide v0
.end method

.method public final getIPv4Only()Z
    .registers 4

    .line 1016
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1016
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x1f

    .line 1016
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    const/4 v2, 0x1

    if-ne v1, v2, :cond_d

    const/4 v2, 0x1

    return v2

    :cond_d
    const/4 v2, 0x0

    return v2
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final getIdentity()[B
    .registers 6

    .line 618
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 618
    .local v0, "$r2":Lzmq/SocketBase;, ""
    const/4 v2, 0x5

    .line 618
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockoptx(I)Ljava/lang/Object;

    move-result-object v1

    .local v1, "$r1":Ljava/lang/Object;, ""
    move-object v4, v1

    check-cast v4, [B

    move-object v3, v4

    .local v3, "$r3":[B, ""
    return-object v3
    .end local v1    # "$r1":Ljava/lang/Object;, ""
    .end local v3    # "$r3":[B, ""
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public final getLinger()J
    .registers 6

    .line 406
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 406
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x11

    .line 406
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
    .end local v3    # "$l1":J, ""
.end method

.method public final getMaxMsgSize()J
    .registers 8

    .line 485
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 485
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x16

    .line 485
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockoptx(I)Ljava/lang/Object;

    move-result-object v1

    .local v1, "$r2":Ljava/lang/Object;, ""
    move-object v4, v1

    check-cast v4, Ljava/lang/Long;

    move-object v3, v4

    .line 485
    .local v3, "$r3":Ljava/lang/Long;, ""
    invoke-virtual {v3}, Ljava/lang/Long;->longValue()J

    move-result-wide v5

    .local v5, "$l0":J, ""
    return-wide v5
    .end local v1    # "$r2":Ljava/lang/Object;, ""
    .end local v5    # "$l0":J, ""
    .end local v3    # "$r3":Ljava/lang/Long;, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final getMulticastHops()J
    .registers 6

    .line 721
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 721
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x19

    .line 721
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v3    # "$l1":J, ""
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final getRate()J
    .registers 6

    .line 648
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 648
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x8

    .line 648
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getRcvHWM()J
    .registers 6

    .line 517
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 517
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x18

    .line 517
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
    .end local v3    # "$l1":J, ""
.end method

.method public final getReceiveBufferSize()J
    .registers 6

    .line 886
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 886
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0xc

    .line 886
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
.end method

.method public final getReceiveTimeOut()I
    .registers 4

    .line 742
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 742
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x1b

    .line 742
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    return v1
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getReconnectIVL()J
    .registers 6

    .line 436
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 436
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x12

    .line 436
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v1    # "$i0":I, ""
    .end local v3    # "$l1":J, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final getReconnectIVLMax()J
    .registers 6

    .line 469
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 469
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x15

    .line 469
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getRecoveryInterval()J
    .registers 6

    .line 669
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 669
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x9

    .line 669
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
.end method

.method public final getSendBufferSize()J
    .registers 6

    .line 864
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 864
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0xb

    .line 864
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v3    # "$l1":J, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getSendTimeOut()I
    .registers 4

    .line 765
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 765
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x1c

    .line 765
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    return v1
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getSndHWM()J
    .registers 6

    .line 501
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 501
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x17

    .line 501
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final getSwap()J
    .registers 3
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    const-wide/16 v0, -0x1

    return-wide v0
.end method

.method public getTCPKeepAlive()I
    .registers 4

    .line 1036
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1036
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x22

    .line 1036
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    return v1
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public getTCPKeepAliveCount()J
    .registers 6

    .line 816
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 816
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x23

    .line 816
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v1    # "$i0":I, ""
    .end local v3    # "$l1":J, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public getTCPKeepAliveIdle()J
    .registers 6

    .line 855
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 855
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x24

    .line 855
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v3    # "$l1":J, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public getTCPKeepAliveInterval()J
    .registers 6

    .line 836
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 836
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x25

    .line 836
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v3    # "$l1":J, ""
    .end local v1    # "$i0":I, ""
.end method

.method public getTCPKeepAliveSetting()J
    .registers 6

    .line 797
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 797
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x22

    .line 797
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    int-to-long v3, v1

    .local v3, "$l1":J, ""
    return-wide v3
    .end local v3    # "$l1":J, ""
    .end local v1    # "$i0":I, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
.end method

.method public final getType()I
    .registers 4

    .line 397
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 397
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0x10

    .line 397
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    return v1
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public final hasMulticastLoop()Z
    .registers 2
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    const/4 v0, 0x0

    return v0
.end method

.method public final hasReceiveMore()Z
    .registers 4

    .line 913
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 913
    .local v0, "$r1":Lzmq/SocketBase;, ""
    const/16 v2, 0xd

    .line 913
    invoke-virtual {v0, v2}, Lzmq/SocketBase;->getsockopt(I)I

    move-result v1

    .local v1, "$i0":I, ""
    const/4 v2, 0x1

    if-ne v1, v2, :cond_d

    const/4 v2, 0x1

    return v2

    :cond_d
    const/4 v2, 0x0

    return v2
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i0":I, ""
.end method

.method public monitor(Ljava/lang/String;I)Z
    .registers 5
    .param p1, "addr"    # Ljava/lang/String;
    .param p2, "events"    # I

    .line 1337
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1337
    .local v0, "$r2":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p1, p2}, Lzmq/SocketBase;->monitor(Ljava/lang/String;I)Z

    move-result v1

    .local v1, "$z0":Z, ""
    return v1
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
    .end local v1    # "$z0":Z, ""
.end method

.method public final recv([BIII)I
    .registers 8
    .param p1, "buffer"    # [B
    .param p2, "offset"    # I
    .param p3, "len"    # I
    .param p4, "flags"    # I

    .line 1274
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1274
    .local v0, "$r2":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p4}, Lzmq/SocketBase;->recv(I)Lzmq/Msg;

    move-result-object v1

    .local v1, "$r3":Lzmq/Msg;, ""
    if-eqz v1, :cond_e

    .line 1277
    const/4 v2, 0x0

    .line 1277
    invoke-virtual {v1, v2, p1, p2, p3}, Lzmq/Msg;->getBytes(I[BII)I

    move-result p2

    .line 1281
    .local p2, "$i0":I, ""
    return p2

    :cond_e
    const/4 v2, -0x1

    return v2
    .end local p2    # "$i0":I, ""
    .end local v1    # "$r3":Lzmq/Msg;, ""
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public final recv()[B
    .registers 3

    .line 1235
    const/4 v1, 0x0

    .line 1235
    invoke-virtual {p0, v1}, Lorg/zeromq/ZMQ$Socket;->recv(I)[B

    move-result-object v0

    .local v0, "$r1":[B, ""
    return-object v0
    .end local v0    # "$r1":[B, ""
.end method

.method public final recv(I)[B
    .registers 6
    .param p1, "flags"    # I

    .line 1247
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1247
    .local v0, "$r1":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p1}, Lzmq/SocketBase;->recv(I)Lzmq/Msg;

    move-result-object v1

    .local v1, "$r2":Lzmq/Msg;, ""
    if-eqz v1, :cond_d

    .line 1250
    invoke-virtual {v1}, Lzmq/Msg;->data()[B

    move-result-object v2

    .line 1254
    .local v2, "$r3":[B, ""
    return-object v2

    .line 1253
    :cond_d
    invoke-direct {p0}, Lorg/zeromq/ZMQ$Socket;->mayRaise()V

    const/4 v3, 0x0

    return-object v3
    .end local v1    # "$r2":Lzmq/Msg;, ""
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v2    # "$r3":[B, ""
.end method

.method public final recvByteBuffer(Ljava/nio/ByteBuffer;I)I
    .registers 7
    .param p1, "buffer"    # Ljava/nio/ByteBuffer;
    .param p2, "flags"    # I

    .line 1293
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1293
    .local v0, "$r2":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p2}, Lzmq/SocketBase;->recv(I)Lzmq/Msg;

    move-result-object v1

    .local v1, "$r3":Lzmq/Msg;, ""
    if-eqz v1, :cond_14

    .line 1296
    invoke-virtual {v1}, Lzmq/Msg;->data()[B

    move-result-object v2

    .line 1296
    .local v2, "$r4":[B, ""
    invoke-virtual {p1, v2}, Ljava/nio/ByteBuffer;->put([B)Ljava/nio/ByteBuffer;

    .line 1297
    invoke-virtual {v1}, Lzmq/Msg;->size()I

    move-result p2

    .line 1301
    .local p2, "$i0":I, ""
    return p2

    .line 1300
    :cond_14
    invoke-direct {p0}, Lorg/zeromq/ZMQ$Socket;->mayRaise()V

    const/4 v3, -0x1

    return v3
    .end local v2    # "$r4":[B, ""
    .end local p2    # "$i0":I, ""
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
    .end local v1    # "$r3":Lzmq/Msg;, ""
.end method

.method public final recvStr()Ljava/lang/String;
    .registers 3

    .line 1308
    const/4 v1, 0x0

    .line 1308
    invoke-virtual {p0, v1}, Lorg/zeromq/ZMQ$Socket;->recvStr(I)Ljava/lang/String;

    move-result-object v0

    .local v0, "$r1":Ljava/lang/String;, ""
    return-object v0
    .end local v0    # "$r1":Ljava/lang/String;, ""
.end method

.method public final recvStr(I)Ljava/lang/String;
    .registers 6
    .param p1, "flags"    # I

    .line 1318
    invoke-virtual {p0, p1}, Lorg/zeromq/ZMQ$Socket;->recv(I)[B

    move-result-object v0

    .local v0, "$r2":[B, ""
    if-eqz v0, :cond_e

    .line 1321
    new-instance v1, Ljava/lang/String;

    .local v1, "$r3":Ljava/lang/String;, ""
    sget-object v2, Lorg/zeromq/ZMQ;->CHARSET:Ljava/nio/charset/Charset;

    .line 1321
    .local v2, "$r1":Ljava/nio/charset/Charset;, ""
    invoke-direct {v1, v0, v2}, Ljava/lang/String;-><init>([BLjava/nio/charset/Charset;)V

    .line 1324
    return-object v1

    :cond_e
    const/4 v3, 0x0

    return-object v3
    .end local v0    # "$r2":[B, ""
    .end local v1    # "$r3":Ljava/lang/String;, ""
    .end local v2    # "$r1":Ljava/nio/charset/Charset;, ""
.end method

.method public final send(Ljava/lang/String;)Z
    .registers 6
    .param p1, "data"    # Ljava/lang/String;

    .line 1185
    sget-object v0, Lorg/zeromq/ZMQ;->CHARSET:Ljava/nio/charset/Charset;

    .line 1185
    .local v0, "$r2":Ljava/nio/charset/Charset;, ""
    invoke-virtual {p1, v0}, Ljava/lang/String;->getBytes(Ljava/nio/charset/Charset;)[B

    move-result-object v1

    .line 1185
    .local v1, "$r3":[B, ""
    const/4 v3, 0x0

    .line 1185
    invoke-virtual {p0, v1, v3}, Lorg/zeromq/ZMQ$Socket;->send([BI)Z

    move-result v2

    .local v2, "$z0":Z, ""
    return v2
    .end local v1    # "$r3":[B, ""
    .end local v0    # "$r2":Ljava/nio/charset/Charset;, ""
    .end local v2    # "$z0":Z, ""
.end method

.method public final send(Ljava/lang/String;I)Z
    .registers 6
    .param p1, "data"    # Ljava/lang/String;
    .param p2, "flags"    # I

    .line 1193
    sget-object v0, Lorg/zeromq/ZMQ;->CHARSET:Ljava/nio/charset/Charset;

    .line 1193
    .local v0, "$r2":Ljava/nio/charset/Charset;, ""
    invoke-virtual {p1, v0}, Ljava/lang/String;->getBytes(Ljava/nio/charset/Charset;)[B

    move-result-object v1

    .line 1193
    .local v1, "$r3":[B, ""
    invoke-virtual {p0, v1, p2}, Lorg/zeromq/ZMQ$Socket;->send([BI)Z

    move-result v2

    .local v2, "$z0":Z, ""
    return v2
    .end local v0    # "$r2":Ljava/nio/charset/Charset;, ""
    .end local v1    # "$r3":[B, ""
    .end local v2    # "$z0":Z, ""
.end method

.method public final send([B)Z
    .registers 4
    .param p1, "data"    # [B

    .line 1197
    const/4 v1, 0x0

    .line 1197
    invoke-virtual {p0, p1, v1}, Lorg/zeromq/ZMQ$Socket;->send([BI)Z

    move-result v0

    .local v0, "$z0":Z, ""
    return v0
    .end local v0    # "$z0":Z, ""
.end method

.method public final send([BI)Z
    .registers 7
    .param p1, "data"    # [B
    .param p2, "flags"    # I

    .line 1205
    new-instance v0, Lzmq/Msg;

    .line 1205
    .local v0, "$r2":Lzmq/Msg;, ""
    invoke-direct {v0, p1}, Lzmq/Msg;-><init>([B)V

    .line 1206
    iget-object v1, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1206
    .local v1, "$r3":Lzmq/SocketBase;, ""
    invoke-virtual {v1, v0, p2}, Lzmq/SocketBase;->send(Lzmq/Msg;I)Z

    move-result v2

    .local v2, "$z0":Z, ""
    if-eqz v2, :cond_f

    .line 1210
    const/4 v3, 0x1

    .line 1210
    return v3

    .line 1209
    :cond_f
    invoke-direct {p0}, Lorg/zeromq/ZMQ$Socket;->mayRaise()V

    const/4 v3, 0x0

    return v3
    .end local v1    # "$r3":Lzmq/SocketBase;, ""
    .end local v2    # "$z0":Z, ""
    .end local v0    # "$r2":Lzmq/Msg;, ""
.end method

.method public final sendByteBuffer(Ljava/nio/ByteBuffer;I)I
    .registers 7
    .param p1, "data"    # Ljava/nio/ByteBuffer;
    .param p2, "flags"    # I

    .line 1221
    new-instance v0, Lzmq/Msg;

    .line 1221
    .local v0, "$r2":Lzmq/Msg;, ""
    invoke-direct {v0, p1}, Lzmq/Msg;-><init>(Ljava/nio/ByteBuffer;)V

    .line 1222
    iget-object v1, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1222
    .local v1, "$r3":Lzmq/SocketBase;, ""
    invoke-virtual {v1, v0, p2}, Lzmq/SocketBase;->send(Lzmq/Msg;I)Z

    move-result v2

    .local v2, "$z0":Z, ""
    if-eqz v2, :cond_12

    .line 1223
    invoke-virtual {v0}, Lzmq/Msg;->size()I

    move-result p2

    .line 1226
    .local p2, "$i0":I, ""
    return p2

    .line 1225
    :cond_12
    invoke-direct {p0}, Lorg/zeromq/ZMQ$Socket;->mayRaise()V

    const/4 v3, -0x1

    return v3
    .end local v1    # "$r3":Lzmq/SocketBase;, ""
    .end local v2    # "$z0":Z, ""
    .end local p2    # "$i0":I, ""
    .end local v0    # "$r2":Lzmq/Msg;, ""
.end method

.method public final sendMore(Ljava/lang/String;)Z
    .registers 6
    .param p1, "data"    # Ljava/lang/String;

    .line 1189
    sget-object v0, Lorg/zeromq/ZMQ;->CHARSET:Ljava/nio/charset/Charset;

    .line 1189
    .local v0, "$r2":Ljava/nio/charset/Charset;, ""
    invoke-virtual {p1, v0}, Ljava/lang/String;->getBytes(Ljava/nio/charset/Charset;)[B

    move-result-object v1

    .line 1189
    .local v1, "$r3":[B, ""
    const/4 v3, 0x2

    .line 1189
    invoke-virtual {p0, v1, v3}, Lorg/zeromq/ZMQ$Socket;->send([BI)Z

    move-result v2

    .local v2, "$z0":Z, ""
    return v2
    .end local v2    # "$z0":Z, ""
    .end local v0    # "$r2":Ljava/nio/charset/Charset;, ""
    .end local v1    # "$r3":[B, ""
.end method

.method public final sendMore([B)Z
    .registers 4
    .param p1, "data"    # [B

    .line 1201
    const/4 v1, 0x2

    .line 1201
    invoke-virtual {p0, p1, v1}, Lorg/zeromq/ZMQ$Socket;->send([BI)Z

    move-result v0

    .local v0, "$z0":Z, ""
    return v0
    .end local v0    # "$z0":Z, ""
.end method

.method public final setAffinity(J)V
    .registers 5
    .param p1, "value"    # J

    .line 609
    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    .line 609
    .local v0, "$r1":Ljava/lang/Long;, ""
    const/4 v1, 0x4

    .line 609
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 610
    return-void
    .end local v0    # "$r1":Ljava/lang/Long;, ""
.end method

.method public final setBacklog(J)V
    .registers 6
    .param p1, "value"    # J

    long-to-int v0, p1

    .line 459
    .local v0, "$i1":I, ""
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 459
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x13

    .line 459
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 460
    return-void
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$i1":I, ""
.end method

.method public final setDecoder(Ljava/lang/Class;)V
    .registers 4
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/lang/Class",
            "<+",
            "Lzmq/DecoderBase;",
            ">;)V"
        }
    .end annotation

    .line 987
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 987
    .local v0, "$r2":Lzmq/SocketBase;, ""
    const/16 v1, 0x3ea

    .line 987
    invoke-virtual {v0, v1, p1}, Lzmq/SocketBase;->setsockopt(ILjava/lang/Object;)V

    .line 988
    return-void
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public setDelayAttachOnConnect(Z)V
    .registers 5
    .param p1, "value"    # Z

    if-eqz p1, :cond_d

    const/4 v0, 0x1

    .line 1069
    .local v0, "$b0":B, ""
    :goto_3
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 1069
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x27

    .line 1069
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 1070
    return-void

    .line 1069
    :cond_d
    const/4 v0, 0x0

    goto :goto_3
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$b0":B, ""
.end method

.method public final setEncoder(Ljava/lang/Class;)V
    .registers 4
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/lang/Class",
            "<+",
            "Lzmq/EncoderBase;",
            ">;)V"
        }
    .end annotation

    .line 979
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 979
    .local v0, "$r2":Lzmq/SocketBase;, ""
    const/16 v1, 0x3e9

    .line 979
    invoke-virtual {v0, v1, p1}, Lzmq/SocketBase;->setsockopt(ILjava/lang/Object;)V

    .line 980
    return-void
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public final setHWM(J)V
    .registers 3
    .param p1, "hwm"    # J

    .line 551
    invoke-virtual {p0, p1, p2}, Lorg/zeromq/ZMQ$Socket;->setSndHWM(J)V

    .line 552
    invoke-virtual {p0, p1, p2}, Lorg/zeromq/ZMQ$Socket;->setRcvHWM(J)V

    .line 553
    return-void
.end method

.method public setIPv4Only(Z)V
    .registers 5
    .param p1, "v4only"    # Z

    if-eqz p1, :cond_d

    const/4 v0, 0x1

    .line 1026
    .local v0, "$b0":B, ""
    :goto_3
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 1026
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x1f

    .line 1026
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 1027
    return-void

    .line 1026
    :cond_d
    const/4 v0, 0x0

    goto :goto_3
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$b0":B, ""
.end method

.method public final setIdentity([B)V
    .registers 3
    .param p1, "identity"    # [B

    .line 639
    const/4 v0, 0x5

    .line 639
    invoke-direct {p0, v0, p1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 640
    return-void
.end method

.method public final setLinger(J)V
    .registers 7
    .param p1, "value"    # J

    .line 427
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .local v0, "$r1":Lzmq/SocketBase;, ""
    long-to-int v1, p1

    .line 427
    .local v1, "$i1":I, ""
    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v2

    .line 427
    .local v2, "$r2":Ljava/lang/Integer;, ""
    const/16 v3, 0x11

    .line 427
    invoke-virtual {v0, v3, v2}, Lzmq/SocketBase;->setsockopt(ILjava/lang/Object;)V

    .line 428
    return-void
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i1":I, ""
    .end local v2    # "$r2":Ljava/lang/Integer;, ""
.end method

.method public final setMaxMsgSize(J)V
    .registers 5
    .param p1, "value"    # J

    .line 491
    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    .line 491
    .local v0, "$r1":Ljava/lang/Long;, ""
    const/16 v1, 0x16

    .line 491
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 492
    return-void
    .end local v0    # "$r1":Ljava/lang/Long;, ""
.end method

.method public final setMulticastHops(J)V
    .registers 4
    .param p1, "value"    # J

    .line 733
    new-instance v0, Ljava/lang/UnsupportedOperationException;

    .line 733
    .local v0, "$r1":Ljava/lang/UnsupportedOperationException;, ""
    invoke-direct {v0}, Ljava/lang/UnsupportedOperationException;-><init>()V

    throw v0
    .end local v0    # "$r1":Ljava/lang/UnsupportedOperationException;, ""
.end method

.method public final setMulticastLoop(Z)V
    .registers 3
    .param p1, "mcast_loop"    # Z
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    .line 712
    new-instance v0, Ljava/lang/UnsupportedOperationException;

    .line 712
    .local v0, "$r1":Ljava/lang/UnsupportedOperationException;, ""
    invoke-direct {v0}, Ljava/lang/UnsupportedOperationException;-><init>()V

    throw v0
    .end local v0    # "$r1":Ljava/lang/UnsupportedOperationException;, ""
.end method

.method public final setRate(J)V
    .registers 4
    .param p1, "value"    # J

    .line 659
    new-instance v0, Ljava/lang/UnsupportedOperationException;

    .line 659
    .local v0, "$r1":Ljava/lang/UnsupportedOperationException;, ""
    invoke-direct {v0}, Ljava/lang/UnsupportedOperationException;-><init>()V

    throw v0
    .end local v0    # "$r1":Ljava/lang/UnsupportedOperationException;, ""
.end method

.method public final setRcvHWM(J)V
    .registers 6
    .param p1, "value"    # J

    long-to-int v0, p1

    .line 523
    .local v0, "$i1":I, ""
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 523
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x18

    .line 523
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 524
    return-void
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$i1":I, ""
.end method

.method public final setReceiveBufferSize(J)V
    .registers 6
    .param p1, "value"    # J

    long-to-int v0, p1

    .line 899
    .local v0, "$i1":I, ""
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 899
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0xc

    .line 899
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 900
    return-void
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$i1":I, ""
.end method

.method public final setReceiveTimeOut(I)V
    .registers 4
    .param p1, "value"    # I

    .line 755
    invoke-static {p1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    .line 755
    .local v0, "$r1":Ljava/lang/Integer;, ""
    const/16 v1, 0x1b

    .line 755
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 756
    return-void
    .end local v0    # "$r1":Ljava/lang/Integer;, ""
.end method

.method public final setReconnectIVL(J)V
    .registers 7
    .param p1, "value"    # J

    .line 442
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .local v0, "$r1":Lzmq/SocketBase;, ""
    long-to-int v1, p1

    .line 442
    .local v1, "$i1":I, ""
    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v2

    .line 442
    .local v2, "$r2":Ljava/lang/Integer;, ""
    const/16 v3, 0x12

    .line 442
    invoke-virtual {v0, v3, v2}, Lzmq/SocketBase;->setsockopt(ILjava/lang/Object;)V

    .line 443
    return-void
    .end local v0    # "$r1":Lzmq/SocketBase;, ""
    .end local v1    # "$i1":I, ""
    .end local v2    # "$r2":Ljava/lang/Integer;, ""
.end method

.method public final setReconnectIVLMax(J)V
    .registers 6
    .param p1, "value"    # J

    long-to-int v0, p1

    .line 475
    .local v0, "$i1":I, ""
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 475
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x15

    .line 475
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 476
    return-void
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$i1":I, ""
.end method

.method public final setRecoveryInterval(J)V
    .registers 4
    .param p1, "value"    # J

    .line 686
    new-instance v0, Ljava/lang/UnsupportedOperationException;

    .line 686
    .local v0, "$r1":Ljava/lang/UnsupportedOperationException;, ""
    invoke-direct {v0}, Ljava/lang/UnsupportedOperationException;-><init>()V

    throw v0
    .end local v0    # "$r1":Ljava/lang/UnsupportedOperationException;, ""
.end method

.method public final setRouterMandatory(Z)V
    .registers 5
    .param p1, "mandatory"    # Z

    if-eqz p1, :cond_d

    const/4 v0, 0x1

    .line 997
    .local v0, "$b0":B, ""
    :goto_3
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 997
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x21

    .line 997
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 998
    return-void

    .line 997
    :cond_d
    const/4 v0, 0x0

    goto :goto_3
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$b0":B, ""
.end method

.method public final setSendBufferSize(J)V
    .registers 6
    .param p1, "value"    # J

    long-to-int v0, p1

    .line 877
    .local v0, "$i1":I, ""
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 877
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0xb

    .line 877
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 878
    return-void
    .end local v0    # "$i1":I, ""
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
.end method

.method public final setSendTimeOut(I)V
    .registers 4
    .param p1, "value"    # I

    .line 778
    invoke-static {p1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    .line 778
    .local v0, "$r1":Ljava/lang/Integer;, ""
    const/16 v1, 0x1c

    .line 778
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 779
    return-void
    .end local v0    # "$r1":Ljava/lang/Integer;, ""
.end method

.method public final setSndHWM(J)V
    .registers 6
    .param p1, "value"    # J

    long-to-int v0, p1

    .line 507
    .local v0, "$i1":I, ""
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 507
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x17

    .line 507
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 508
    return-void
    .end local v0    # "$i1":I, ""
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
.end method

.method public final setSwap(J)V
    .registers 4
    .param p1, "value"    # J
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    .line 578
    new-instance v0, Ljava/lang/UnsupportedOperationException;

    .line 578
    .local v0, "$r1":Ljava/lang/UnsupportedOperationException;, ""
    invoke-direct {v0}, Ljava/lang/UnsupportedOperationException;-><init>()V

    throw v0
    .end local v0    # "$r1":Ljava/lang/UnsupportedOperationException;, ""
.end method

.method public setTCPKeepAlive(I)V
    .registers 4
    .param p1, "optVal"    # I

    .line 1046
    invoke-static {p1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    .line 1046
    .local v0, "$r1":Ljava/lang/Integer;, ""
    const/16 v1, 0x22

    .line 1046
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 1047
    return-void
    .end local v0    # "$r1":Ljava/lang/Integer;, ""
.end method

.method public setTCPKeepAlive(J)V
    .registers 5
    .param p1, "value"    # J

    .line 788
    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    .line 788
    .local v0, "$r1":Ljava/lang/Long;, ""
    const/16 v1, 0x22

    .line 788
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 789
    return-void
    .end local v0    # "$r1":Ljava/lang/Long;, ""
.end method

.method public setTCPKeepAliveCount(J)V
    .registers 5
    .param p1, "value"    # J

    .line 807
    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    .line 807
    .local v0, "$r1":Ljava/lang/Long;, ""
    const/16 v1, 0x23

    .line 807
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 808
    return-void
    .end local v0    # "$r1":Ljava/lang/Long;, ""
.end method

.method public setTCPKeepAliveIdle(J)V
    .registers 5
    .param p1, "value"    # J

    .line 846
    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    .line 846
    .local v0, "$r1":Ljava/lang/Long;, ""
    const/16 v1, 0x24

    .line 846
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 847
    return-void
    .end local v0    # "$r1":Ljava/lang/Long;, ""
.end method

.method public setTCPKeepAliveInterval(J)V
    .registers 5
    .param p1, "value"    # J

    .line 827
    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    .line 827
    .local v0, "$r1":Ljava/lang/Long;, ""
    const/16 v1, 0x25

    .line 827
    invoke-direct {p0, v1, v0}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 828
    return-void
    .end local v0    # "$r1":Ljava/lang/Long;, ""
.end method

.method public final setXpubVerbose(Z)V
    .registers 5
    .param p1, "verbose"    # Z

    if-eqz p1, :cond_d

    const/4 v0, 0x1

    .line 1007
    .local v0, "$b0":B, ""
    :goto_3
    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    .line 1007
    .local v1, "$r1":Ljava/lang/Integer;, ""
    const/16 v2, 0x28

    .line 1007
    invoke-direct {p0, v2, v1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 1008
    return-void

    .line 1007
    :cond_d
    const/4 v0, 0x0

    goto :goto_3
    .end local v1    # "$r1":Ljava/lang/Integer;, ""
    .end local v0    # "$b0":B, ""
.end method

.method public final subscribe([B)V
    .registers 3
    .param p1, "topic"    # [B

    .line 957
    const/4 v0, 0x6

    .line 957
    invoke-direct {p0, v0, p1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 958
    return-void
.end method

.method public final unbind(Ljava/lang/String;)Z
    .registers 4
    .param p1, "addr"    # Ljava/lang/String;

    .line 1181
    iget-object v0, p0, Lorg/zeromq/ZMQ$Socket;->base:Lzmq/SocketBase;

    .line 1181
    .local v0, "$r2":Lzmq/SocketBase;, ""
    invoke-virtual {v0, p1}, Lzmq/SocketBase;->term_endpoint(Ljava/lang/String;)Z

    move-result v1

    .local v1, "$z0":Z, ""
    return v1
    .end local v1    # "$z0":Z, ""
    .end local v0    # "$r2":Lzmq/SocketBase;, ""
.end method

.method public final unsubscribe([B)V
    .registers 3
    .param p1, "topic"    # [B

    .line 971
    const/4 v0, 0x7

    .line 971
    invoke-direct {p0, v0, p1}, Lorg/zeromq/ZMQ$Socket;->setsockopt(ILjava/lang/Object;)V

    .line 972
    return-void
.end method
